...
class MyProblem(ea.Problem):  # 继承Problem父类
    ...
    def punish(self, var):
        ...
        for feaindex, indexinfea in enumerate(var):
            if self.auto2Fixed.fixed[feaindex].feavtk.points()[indexinfea] == \
                    self.auto2Fixed.fixed[feaindex + 1].feavtk.points()[var[feaindex + 1]]:
                return True
        if self.ndl_intersect(var):
            return True
        else:
            return False

    def ndl_intersect(self, var):
        """
        若针之间存在相交，返回True，否则False
        :param var:
        :return:
        """
        for feaindex, indexinfea in enumerate(var):
            tar = self.auto2Fixed.fixed[feaindex].pos
            cylinder = Cylinder(pos=tar, r=5, height=300, axis=self.auto2Fixed.fixed.direct)
            for j in range(feaindex + 1, len(var)):
                jskinpt = self.auto2Fixed.fixed[j].feavtk.points()[var[j]]
                jtar = self.auto2Fixed.fixed[j].pos
                jrealin = NeedleUtils.get_real_inpt(jskinpt, jtar)
                intersects = cylinder.intersectWithLine(jrealin, jtar)
                if len(intersects) > 0:
                    return True
        return False
    ...
    def get_mean_depth(self, var, punishflag):
        ...
        for feaindex, indexinfea in enumerate(var):
            indexinfea = int(indexinfea)
            feaindexthdepth = NeedleUtils.get_dist(self.auto2Fixed.fixed[feaindex].pos,
